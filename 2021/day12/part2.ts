...
function pathfind(nodes: AdjacencyList, cave: string, history: string[] = []) {
  ...
  for (let neighbor of nodes[cave]) {
    if (neighbor !== "start") {
      // We can revisit big caves whenever
      if (IsUpper.test(neighbor)) {
        // console.log(`Visiting cave ${neighbor}, from node ${cave}.`);
        let newResults = pathfind(nodes, neighbor, newHistory);
        results = results.concat(newResults);

      } else {
        const caveCounts = _.mapValues(_.groupBy(newHistory.filter(c => IsLower.test(c))), "length");
        const hasATwoVisit = _.values(caveCounts).some(val => val > 1);

        const count = caveCounts[neighbor] ?? 0;

        if ((hasATwoVisit && count < 1) || (!hasATwoVisit && count < 2)) {
          // console.log(`Visiting cave ${neighbor}, from node ${cave}.`);
          let newResults = pathfind(nodes, neighbor, newHistory);
          results = results.concat(newResults);
        }
