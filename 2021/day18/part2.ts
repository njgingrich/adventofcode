...
export default async function run() {
  ...
  for (let perm of it.permutations(pairs, 2)) {
    let total = add(perm[0], perm[1]).reduce((sum, node) => sum + magnitude(node), 0);
    maxMagnitude = Math.max(maxMagnitude, total);
  }
  return ([...it.permutations(pairs, 2)])
    .map(([first, second]) => add(first, second))
